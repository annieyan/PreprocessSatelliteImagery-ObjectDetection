...
In the case of NOAA data, there are two classes: damaged buildings (class2 )/ non-damaged buildings (class1)
This file produce TF record for training data only.
Big tiffs (2048) are chipped sequentially into 200 x 200.
Augmentation will be applied to chips that contain damaged buildings (class2)
...
if __name__ == "__main__":
    ...
    train_writer = tf.python_io.TFRecordWriter("noaa_train_%s.record" % args.suffix)
    ...
    coords,chips,classes,uids = wv.get_labels_noaa_w_uids(args.json_filepath)
             ...
             #   print('new_classes', new_classes)
                ...
                if (ind_chips < max_chips_per_res and np.array(float_list_value_xmin).any() and np.array(float_list_value_xmax).any() and np.array(float_list_value_ymin).any() and np.array(float_list_value_ymax).any()):
                    ...
                    if SAVE_IMAGES and idx % 5 == 0:
                        ...
                        aug.draw_bboxes(image, new_coords).save('./noaa_train_2class_inspect/img_%s_%s.png'%(name,str(idx)))
                ...
                #If there are no valid bounding boxes, then don't save the image to the TFRecord.
                         ...
                         #           num_aug_this_class=num_aug_this_class + 1
                                    ...
                                    # debug: changed save dir
                                        ...
                                        aug.draw_bboxes(aug_image,boxes_aug[aug_idx]).save('./noaa_augmentation/img_aug_%s_%s_%s_%s.png'%(name, str(idx), str(aug_idx), str(class_id)))
                            # debug
                    ...
                    if AUGMENT and prob < it[0] and MINOR_CLASS_FLAG == False:
                	    ...
                	    # save image + bounding boxes for debug
                                    ...
                                    if idx%100 ==0 and SAVE_IMAGES:
                                        # debug: changed save dir
                                        aug.draw_bboxes(newimg,nb).save('./noaa_augmented/img_aug_%s_%s_%s.png'%(name,extra,it[0]))
