...
def detect_clouds(img,  boxes, classes, uids):
...
if __name__ == "__main__":
    ...
    for res_ind, it in enumerate(res):
        ...
        for fname in tqdm(fnames):
            ...
            im,box,classes_final, uids_chip = wv.chip_image_with_uid(arr,coords[chips==name],classes[chips==name],uids[chips==name] ,it)
                ...
                if (ind_chips < max_chips_per_res and np.array(float_list_value_xmin).any() and np.array(float_list_value_xmax).any() and np.array(float_list_value_ymin).any() and np.array(float_list_value_ymax).any()):
                    ...
                    if idx < split_ind:
                        ...
                        test_chips+=1
                         ...
                         #   aug.draw_bboxes(image, new_coords).save('./harvey_img_inspect_uid_val/img_%s_%s.png'%(name,str(idx)))
                        ...
                        #if SAVE_IMAGES:
                                    # debug: changed save dir
                         #   aug.draw_bboxes(image, new_coords).save('./harvey_img_inspect_uid_train/img_%s_%s.png'%(name,str(idx)))
