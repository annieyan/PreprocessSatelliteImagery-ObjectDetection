...
Training data is created by chipping the large (2048) tif files into smaller(200) tif files
centered with each bounding boxes 
'''
...
# TODO: remove this function once the data get manually cleaned
...
if __name__ == "__main__":
    ...
    for res_ind, it in enumerate(res):
        ...
        for fname in tqdm(fnames):
            ...
            # debug
            # changed chipping to cropping from the bbox center
            #im,box,classes_final = wv.chip_image(arr,coords[chips==name],classes[chips==name],it)
            im,box,classes_final = wv.crop_from_center(arr,coords[chips==name],classes[chips==name],it)
                    ...
                    if AUGMENT and prob < it[0] and MINOR_CLASS_FLAG == False:
                        ...
                        for extra in range(3):
                            ...
                            newimg = (newimg).astype(np.uint8)
                                ...
                                #debug
                                # changed save dir
                             #   Image.fromarray(newimg).save('./augmented_img_ms_cropcenter/img_%s_%s_%s.png'%(name,extra,it[0]))
 ...
 #Check to make sure that the TF_Example has valid bounding boxes.  
                #If there are no valid bounding boxes, then don't save the image to the TFRecord.
                                float_list_value_xmin = tf_example_aug2.features.feature['image/object/bbox/xmin'].float_list.value
                                float_list_value_xmax = tf_example_aug2.features.feature['image/object/bbox/xmax'].float_list.value
                                float_list_value_ymin = tf_example_aug2.features.feature['image/object/bbox/ymin'].float_list.value
                                float_list_value_ymax = tf_example_aug2.features.feature['image/object/bbox/ymax'].float_list.value
                	    ...
                	    # save image + bounding boxes for debug
                                    ...
                                    if idx%100 ==0 and SAVE_IMAGES:
                                        # debug: changed save dir
                                        aug.draw_bboxes(newimg,nb).save('./harvey_augmented_crop_center_nondamaged/img_aug_%s_%s_%s.png'%(name,extra,it[0]))
        if res_ind == 0:
