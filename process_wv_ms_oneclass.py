...
'''
This script creates one-class (damaged buildings) training / dev data 
using Digital Globe data + MS building footprints.
The training data included chips that contain at least one building (damaged or NOT)
    ...
    for res_ind, it in enumerate(res):
        ...
        for fname in tqdm(fnames):
            ...
            for idx, image in enumerate(im):
                ...
                # debug
                # Remove small chips that do not contain any bboxes (damaged + non-damaged)
                # first check validity of bboxes, and then remove images that have 0 bboxes left
                #image, new_coords, new_classes = aug.check_bbox_validity(image_rmcloud, new_coords_rmcloud,  new_classes_rmcloud)
                ...
                # if this chip only contains non-damaged buildings, but not damaged buildings.... 
                #else:
                 #   tf_example = tfr.to_tf_example(image, np.array([[0,0,0,0]]), np.array([0]))
                ...
                if idx < split_ind:
                    ...
                    if SAVE_IMAGES:
                        ...
                        aug.draw_bboxes(image, new_coords[new_classes ==1]).save('./harvey_ms_img_inspect_val_1class_noclean/img_%s_%s.png'%(name,str(idx)))
                    ...
                    if SAVE_IMAGES:
                                    # debug: changed save dir
                        aug.draw_bboxes(image, new_coords[new_classes ==1]).save('./harvey_ms_img_inspect_train_1class_noclean/img_%s_%s.png'%(name,str(idx)))
                ...
                #If there are no valid bounding boxes, then don't save the image to the TFRecord.
                            ...
                            float_list_value_ymax = tf_example_aug.features.feature['image/object/bbox/ymax'].float_list.value
                                ...
                                # debug
                                #num_aug = 0
                             # for augmentation, augment those only have damaged building bboxes
                         ...
                         #           num_aug_this_class=num_aug_this_class + 1
                                ...
                                if aug_idx%10 == 0 and SAVE_IMAGES:
                                    ...
                                    aug.draw_bboxes(aug_image,boxes_aug[aug_idx]).save('./MS_expand_aug_random_200_1class/img_aug_%s_%s_%s_%s.png'%(name, str(idx), str(aug_idx), str(class_id)))
                            # debug
